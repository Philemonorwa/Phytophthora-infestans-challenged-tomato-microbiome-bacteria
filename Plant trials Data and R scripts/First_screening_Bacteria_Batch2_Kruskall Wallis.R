...
#Read data
library(readxl)
BDS_Firstscreening <- read_excel("~/Ad_Planta_Trials/Raw_data/Firstscreening_P.infestans_Batch2.xlsx")
#View(BDS_Firstscreening)
#remove unnecessary columns
BDS_FS_Batch2 <- BDS_Firstscreening [,c(1:4)]

#Find all missing rel. inf. values
# This converts all non.numeric rel. inf. values to NA and returns the respective row number
x_nonum <- which(is.na(as.numeric(BDS_FS_Batch2$Diseased_leaf_area)))

BDS_FS_Batch2[x_nonum,]
...
BDS_FS_Batch2$Diseased_leaf_area <- as.numeric(BDS_FS_Batch2$Diseased_leaf_area)
...
#1.
df_mean_BDS_FS_Batch2 <- BDS_FS_Batch2 %>%
  group_by(Treatments, Plant) %>% summarize(Diseased_leaf_area = mean(Diseased_leaf_area, na.rm = TRUE))

### If nececary, rename them
#library(dplyr)

df_mean_BDS_FS_Batch2$Treatments %>% unique()

df_mean_BDS_FS_Batch2$Treatments <- recode(df_mean_BDS_FS_Batch2$Treatments,
                                           "Pb107" = 'Chryseobacterium sp._Pb107', 
                                           "Pb396" = 'Bacillus sp._Pb396',
                                           "Pb444" = 'Bacillus sp._Pb444',
                                           "Pb176" = 'Pseudomonas sp._Pb176',
                                           "Pb151" = 'Pseudomonas sp._Pb151', 
                                           "Pb377" = 'Pseudomonas sp._Pb377',
                                           "Pb160" = 'Paenibacillus sp._Pb160',
                                           "Pb171" = 'Sphingomonas sp._Pb171',
                                           "Pb301" = 'Streptomyces sp._Pb301',
                                           "Pb557" = 'Streptomyces sp._Pb557',
                                           "Pb565" = 'Rhizobiaceae sp._Pb565',
                                           "Cs" = 'Copper standard',
                                            "Pc" = 'Diseased control',
                                            "Nc" = 'Healthy control')
### Reorder them
df_mean_BDS_FS_Batch2$Treatments <- factor(df_mean_BDS_FS_Batch2$Treatments, 
                                            levels = c('Chryseobacterium sp._Pb107', 
                                                       'Bacillus sp._Pb396',
                                                      'Bacillus sp._Pb444',
                                                      'Paenibacillus sp._Pb160',
                                                      'Pseudomonas sp._Pb176',
                                                       'Pseudomonas sp._Pb151', 
                                                      'Pseudomonas sp._Pb377',
                                                       'Sphingomonas sp._Pb171',
                                                       'Streptomyces sp._Pb301',
                                                      'Streptomyces sp._Pb557',
                                                      'Rhizobiaceae sp._Pb565',
                                                      'Copper standard',
                                                        'Diseased control',
                                                       'Healthy control'))
unique(df_mean_BDS_FS_Batch2$Treatments)
#Use to order the table according to the factor levels
df_mean_BDS_FS_Batch2 <- with(df_mean_BDS_FS_Batch2, df_mean_BDS_FS_Batch2[order(Treatments),])


Summarydata <- df_mean_BDS_FS_Batch2 %>%
  group_by(Treatments) %>%
...
Alldatalist_BDS_FS_Batch2 <- list("Raw data" = BDS_FS_Batch2,
                                "Mean per plant" = df_mean_BDS_FS_Batch2, 
                               "Statistics" = Summarydata)
write.xlsx(Alldatalist_BDS_FS_Batch2, file = "Excelfile_Alldatalist_BDS_FS_Batch2.xlsx")
...
library(psych)
describeBy(df_mean_BDS_FS_Batch2$Diseased_leaf_area,df_mean_BDS_FS_Batch2$Treatments)
...
kruskal.test(df_mean_BDS_FS_Batch2$Diseased_leaf_area~df_mean_BDS_FS_Batch2$Treatments)
...
treatments_list <- unique(df_mean_BDS_FS_Batch2$Treatments)
...
ggsave(filename = "BDS_Firstscreening.png", plot = z, path = "~/Ad_Planta_Trials/Outputfiles/Plots", dpi = 300)
